...
3. **Fase C — Recordatorio con botones**: cron 24h manda botones (WA interactive / TG inline) + enlace mágico; callbacks `conf:<id>` en ambos webhooks → `confirmed` + gracias. **Incluye fix del gotcha de Fase 8**: `get_due_reminders` exige que el cliente tenga conversación → citas creadas por staff o por web de clientes que nunca chatearon JAMÁS reciben recordatorio (silencioso). El enlace mágico da el canal alternativo; la RPC debe dejar de omitirlas (marcar `skipped_no_channel` o similar en vez de invisibilidad total). Validación: seed de cita en ventana + corrida del cron + callback simulado + caso "cliente sin conversación".
4. **Fase D — Checklist onboarding + empty states**: componente de checklist derivado del estado real (queries server-side), % y CTAs; empty states en Agenda/Chats/Clientes. **Incluye gotcha de Fase 8**: sin filas en `staff_schedules` el diálogo de cita dice "Sin horarios disponibles" sin explicar la causa — el empty state debe decir "configura la disponibilidad del equipo" con CTA. Validación: Playwright con org nueva (0%) y org completa (100%).
