...
### 2026-08-05 (Fases 4-5): `revoke ... from public` NO cierra una función en Supabase
- **Lo que parecía suficiente y no lo es**: `revoke all on function f from public; grant execute on f to service_role;`. Supabase concede EXECUTE a **`anon` y `authenticated`** por DEFAULT PRIVILEGES en cada función nueva de `public`, y eso es un grant **directo a esos roles**, no a PUBLIC: el revoke no lo toca. Ocho funciones de cron de las Fases 4 y 5 quedaron **ejecutables con la anon key** —que es pública y viaja en el navegador—, incluidas las que devuelven **teléfonos de clientes y correos de negocios de TODAS las orgs**.
- **Y al arreglarlo apareció el matiz inverso**: en tres funciones PREEXISTENTES (`get_due_client_reminders`, `claim_client_reminder`, `merge_duplicate_clients`) el permiso venía de **PUBLIC** (`=X/postgres` en `pg_proc.proacl`), así que revocar de `anon, authenticated` **no cambió nada** y la llamada seguía devolviendo datos. `get_due_client_reminders` exponía mensajes, nombres y teléfonos de clientes de todas las organizaciones: **fuga activa en producción**, ajena a este PRP, corregida aquí.
- **Regla**: toda función solo-cron necesita `revoke execute ... from public, anon, authenticated` + `grant ... to service_role`, **las dos fuentes**. Y verificarlo **llamándola con la anon key**, no leyendo el DDL: `has_function_privilege('anon', …)` es la comprobación rápida, la llamada real es la prueba.
- **El patrón ya estaba documentado en el repo** (`20260704040100_fase5_revoke_reminder_rpcs.sql`) y aun así se repitió. Cuando existe una migración de "blindaje de grants", hay que releerla al crear RPCs nuevas.

### 2026-08-05 (Fase 5): un cero no prueba nada sin control positivo
- **La trampa**: la cola de conversaciones frías devolvía `0` con la capacidad apagada y `0` con la capacidad encendida. Lo primero es lo que se busca; lo segundo también sería el resultado de una consulta **rota**. Hubo que **sembrar** una conversación fría real (cliente sin ninguna cita, 3 mensajes, 10 días de silencio) para comprobar que la consulta la encuentra. Lo mismo con las cifras del resumen: todo a cero no distingue "no pasó nada ayer" de "la consulta no cuenta nada".
- **Regla**: toda consulta de cola o de agregado necesita **un caso que SÍ debe aparecer**, no solo la comprobación de que está vacía cuando toca.
- **El reclamo va antes del envío también aquí, y NO se reintenta**: en WhatsApp, fuera de la ventana de 24 h un mensaje free-form puede fallar en silencio (no hay plantillas HSM aprobadas). Reintentar cada día acabaría acosando al cliente en cuanto la ventana se reabriera. Un intento por conversación y punto — verificado: tras un envío fallido queda marcada y la segunda corrida ya no la toma.
- **El día del resumen se calcula en la zona horaria de la sucursal**, no en UTC: si no, el "resumen de ayer" de un negocio de México parte el día por la mitad.
- **`daily_report_runs` es tabla y no columna** porque el reclamo tiene que ser atómico e histórico: el insert con clave primaria compuesta (org, día) **es** el candado.

### 2026-08-05 (Fase 4): reutilizar una validación "parecida" rompió el CSAT entero
- **El bug**: `record_csat` reutilizaba `_resolve_chat_appointment`, la misma validación de propiedad que usan confirmar/cancelar/reagendar. Parecía lo correcto — misma comprobación, cero duplicación. Pero esa función **exige que la cita sea FUTURA y esté activa** (`starts_at > now()` → `not_actionable`), porque nació para acciones sobre citas por venir. **La encuesta se manda justo DESPUÉS de la cita**, así que habría rechazado el **100 %** de las calificaciones, en silencio y en producción. Corregido: se valida la misma propiedad (cita de ESE cliente en ESE canal, por teléfono canónico) sin el requisito de "accionable".
- **Regla**: antes de reutilizar una función de validación, leer **qué más** valida además de lo que te interesa. "Valida lo mismo que necesito" es distinto de "valida solo lo que necesito". Un helper compartido arrastra las precondiciones de su caso de uso original.
- **Detección**: no lo cazó typecheck, ni lint, ni el DDL en verde. Salió al llamar a la RPC de verdad con una cita real ya pasada. Un test que hubiera usado una cita futura —lo "natural" al escribirlo— habría pasado y dejado el bug vivo.
- **CSAT en WhatsApp: 3 botones, no 5.** El máximo son 3 reply buttons y partir la escala en dos mensajes es peor experiencia. Se mapean Excelente/Bien/Mal a 5/3/1, que siguen siendo válidos en la escala de la BD.
- **El job de calificación se sacó del cron a `scoring-job.ts`**: el endpoint ya multiplexa seis trabajos y roza las 500 líneas, y encima así se puede ejercitar aislado sin disparar de paso recordatorios y correos reales a clientes.

### 2026-08-05 (Fase 3): leer la imagen NO basta — hay que decirle al agente qué hacer con ella
- **El bug que solo salió probando de verdad**: con visión encendida, el agente leyó un comprobante impecable ($850.00 MXN, fecha, beneficiario) y respondió *"Lamento informarte que no puedo ayudarte con eso"*. La lectura estaba bien; el prompt no contemplaba archivos, así que el modelo lo trató como fuera de ámbito. **Una capacidad que funciona a nivel técnico puede ser inútil a nivel de producto.** Se añadió un bloque de reglas de ARCHIVOS al prompt — **condicional**, solo si hay `media_text` en el historial, para que con las capacidades apagadas el prompt siga siendo idéntico al de siempre. Tras el arreglo: *"Gracias por el comprobante, confirmo el monto de $850.00 MXN con fecha del 4 de agosto de 2026"*.
- **Sin tocar `get_agent_context` no había Fase 3**: el historial del agente sale de ahí, así que `media_text` tenía que viajar en él. Se reescribió la función entera partiendo de la definición VIVA (`pg_get_functiondef`), no de la migración del repo, y con los dos añadidos marcados con comentario. Verificado que sigue devolviendo servicios, mensajes, tz y `should_respond`, no solo los campos nuevos.
- **La transcripción no puede ir por OpenRouter** (confirmado): necesita Groq u OpenAI, es decir, otra API key. Mientras no exista, la capacidad **se queda deshabilitada en el panel y la Server Action la rechaza**, en vez de dejar que el dueño la encienda y crea que su agente escucha los audios. Sin key, el sistema degrada al aviso de siempre.
- **La imagen se pasa como bytes**, no como URL firmada: la URL caduca y expondría el archivo a un tercero. Y se fuerza `openai/gpt-4o-mini` para la lectura: `agent_configs.model` lo fija el panel SUPERADMIN y no se puede asumir que tenga visión.
